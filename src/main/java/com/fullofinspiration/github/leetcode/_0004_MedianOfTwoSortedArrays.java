    ...
    class Solution {
        public double findMedianSortedArrays(int[] nums1, int[] nums2) {
            ...
            while (low <= high) {
                ...
                int partitionB = nums1.length + nums2.length - partitionA;
                int aLeftVal = partitionA == 0 ? Integer.MIN_VALUE : nums1[(partitionA - 1) / 2];
                int aRightVal = partitionA == nums1.length * 2 ? Integer.MAX_VALUE : nums1[partitionA / 2];
                int bLeftVal = partitionB == 0 ? Integer.MIN_VALUE : nums2[(partitionB - 1) / 2];
                int bRightVal = partitionB == nums2.length * 2 ? Integer.MAX_VALUE : nums2[partitionB / 2];
                if (aLeftVal > bRightVal) {
                    high = partitionA - 1;
                } else if (bLeftVal > aRightVal) {
                    low = partitionA + 1;
                } else {
                    return (double) (Math.max(aLeftVal, bLeftVal) + Math.min(aRightVal, bRightVal)) / 2;
                }
